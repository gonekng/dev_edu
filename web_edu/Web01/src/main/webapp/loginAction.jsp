...
<%
	...
	String[] result = userDAO.login(user.getUserID(), user.getUserPW());
	
	// 로그인 함수 실행 결과값에 따라서 화면에 출력할 스크립트 생성
	if(result[0].equals("1")){ // 로그인 성공
		
		session.setAttribute("userID", user.getUserID()); // 세션에 userID 넣기
		session.setAttribute("userName", result[1]); // 세션에 userName 넣기
		PrintWriter script = response.getWriter();
		script.println("<script>");
		script.println("location.href='index.jsp'");
		script.println("</script>");
		
	} else if(result[0].equals("0")){ // 로그인 실패
	...
	} else if(result[0].equals("-1")){ // 아이디가 없음
	...
	} else if(result[0].equals("-2")){ // 데이터베이스 오류
